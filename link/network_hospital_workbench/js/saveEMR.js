...
//点击提交病历，先填写是否合格
function saveIsOK(event){
	$(event).attr("disabled","disabled");
	$(event).css("cursor","wait");
 	var checked =$("#u-bhg").is(":checked");
 	var pl_content=$("#pl_content").val();
 	/* 如果选中不合格，验证必须填写理由 */
 	if(checked){
 		if($.trim(pl_content) == null || $.trim(pl_content) == ""){
			createPopWindow("请填写不合格理由");
			$(event).removeAttr("disabled");
			$(event).css("cursor","pointer");
			return false;
		}else if($.trim(pl_content).length>400){
			createPopWindow("不合格理由要在400字以内");
			$(event).removeAttr("disabled");
			$(event).css("cursor","pointer");
			return false;
		}else{
			sendIsOK(0,pl_content,event);
		}
 	}else{
 		sendIsOK(1,pl_content,event);
 	}
}
//发送是否合格及理由
function sendIsOK(IsEligible,pl_content,event){
	var OrderNumber = $("#OrderNumber").val();
	$.post("order/evaluateOrder",{//提交评论
			OrderNumber:OrderNumber,IsEligible:IsEligible,EvaluateContent:pl_content,r:Math.random()
			},function(data){
				if(data.Status==200){								
					saveEMR(event);
				}else{
					createPopWindow("操作失败");
					$(event).removeAttr("disabled");
    				$(event).css("cursor","pointer");
				}
	});	
