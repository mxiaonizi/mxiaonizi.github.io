...
//无效选项隐藏
function displaySecData(event){
	if($(event).val()==1){
	   $(event).parent().parent().children().hide();
	   $(event).parent().show();
	}else{
...
function addTr(event){
	var table=$(event).parent().prev();
	var tr=table.find("tr").eq(1);
	var trClone=tr.clone(true);
	//清空这一行的所有可能状态
	trClone.find("label").remove();	
	trClone.find("input").val("").removeClass("errorLepuInfo");
	trClone.find("input[type=text]").attr("noedit",false);
	trClone.find("td").removeClass("validLepuInfoBox");
	trClone.appendTo(table);
	
	//新加后，根据排序，重新命名后，不同名字的输入框才能分别校验
	var allergySubstance=table.find(".EMRdata[lepuEMR-ID='HR51_01_049']");
	var sortNumber=0;
	allergySubstance.each(function(){
		var name='HR51_01_049_'+ ++sortNumber;
		$(this).attr("name",name);		
	});	
...
function updateDisease(){
	var diagnosisItem=$("#records .EMRdata[lepuEMR-ID='HR55_02_057_04']");
	var diagArray=[];
	diagnosisItem.each(function(){
		if($(this).val()!=""){
			diagArray.push($(this).val());
		}else if(diagnosisItem.length>1){
			$(this).parent().remove();					
		}		
	});	
	$(".diagnosisArray").text(diagArray.join("，"));
}
// 电子处方 删除一行
...
//勾选是否戒酒
function radioBox_drink(event){
	radioBox(event);
	var dataInput=$(event).parent().find("input[lepuEMR-ID='HR51_01_189_01']");
	if($(event).parent().find("input:checked").next().next().text()=="是"){
		dataInput.val(true);
	}else{
		dataInput.val(false);
	}
...
function calDate(event){
	var years=$(event).val();
	var today=new Date();
	var diseaseDate=today.toLocaleDateString().split("/");
	diseaseDate[0]=diseaseDate[0]-years;
	if(diseaseDate[1]<10){diseaseDate[1]="0"+diseaseDate[1]};
	if(diseaseDate[2]<10){diseaseDate[2]="0"+diseaseDate[2]};	
	$(event).parent().prev().children().val(diseaseDate.join("-"));	
}

//计算患病年限 
function calYears(event){
	if($(event).val()!=""){
		var date=$(event).val().split("-");
		var today=new Date();	
		var years=today.getFullYear()-date[0];
		$(event).parent().next().children().val(years);
	}	
}
